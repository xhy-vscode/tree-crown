...
## 适合申请高额度 Token Plan 的原因

该项目需要持续处理高分辨率航拍图，单张图可能包含数百到上千个树冠候选。每轮处理都涉及图像切片、候选网格拼接、视觉模型判断、JSON 结构化输出、失败重试、人工复核反馈和批量迭代训练。随着数据集扩大，视觉推理、Agent 自动化标注、模型评估和结果解释会持续消耗较高 token 和图像额度。
